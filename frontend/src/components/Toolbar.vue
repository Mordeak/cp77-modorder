<template>
  <div class="toolbar">
    ...
    <div class="mo2-toggle" :class="{ active: modStructure === 'MO2' }" @click="$emit('toggle-mo2')" title="Toggle MO2 Mod Organizer 2 support (experimental)">
...
.toolbar button:disabled:hover { border-color: transparent; box-shadow: none; }
.btn-icon { font-size: 15px; line-height: 1; }
.btn-label { font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px; color: var(--cp-dim); }
.toolbar button:hover .btn-label { color: var(--cp-fg); }
.toolbar button:disabled .btn-label { opacity: 0.4; }
...
.mo2-toggle:hover { border-color: var(--cp-focus); }
.mo2-toggle.active { border-color: var(--cp-focus); box-shadow: 0 0 6px rgba(0,229,255,0.25); }
...
.mo2-tag em { font-style: normal; color: var(--cp-focus); opacity: 0.7; }
.mo2-toggle.active .mo2-tag { color: var(--cp-fg); }
...
.mo2-toggle .btn-label { color: var(--cp-dim); }
.mo2-toggle.active .btn-label { color: var(--cp-focus); }
</style>
